...
def parse_infobox(infobox, title, path):
...
def create_infobox_dic(wikiarticle_path, infobox_path, df):
    ...
    for line in fileinput.input(wikiarticle_path, openhook=fileinput.hook_compressed): #hook_compressed hook_encoded('cp65001')
        article += line
        if '</page>' in line:
            ...
            if infobox:
                title, counter1, counter2, counter3 = parse_infobox(infobox, article_title, infobox_path)
                df = df.append(
...
if __name__ == '__main__':
    ...
    path, df = create_infobox_dic('C:/Users/danielak/Desktop/Dokumente Daniela/UNI/FIZ/Second_Task/data/wiki_dump_long.txt', 'C:/Users/danielak/Desktop/Dokumente Daniela/UNI/FIZ/Second_Task/infobox_file/' + str(datetime.datetime.now().month) + str(datetime.datetime.now().day) + 'infobox.txt',df_comp)
    print(df)
